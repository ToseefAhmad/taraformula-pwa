...
export const usePriceSummary = (props = {}) => {
    ...
    const { error, loading, data, refetch } = useQuery(getPriceSummaryQuery, {
    ...
    return {
        ...
        hasError: !!error,
        hasItems: data && data.cart && data.cart.items && !!data.cart.items.length,
        isCheckout,
        isLoading: !!loading,
        flatData: flattenData(data),
        refetchPriceSummary: refetch
    };
};
