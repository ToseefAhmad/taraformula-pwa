...
export const useCartPage = (props = {}) => {
    ...
    // Check if cart data loaded
    const isCartLoaded = (data && data.cart && data.cart.items) || false;
    const shouldShowLoadingIndicator = called && loading && !hasItems;

    const cartItems = useMemo(() => {
        const tempItems = (data && data.cart && data.cart.items) || [];
        return tempItems.filter(e => e !== null);
    }, [data]);
    ...
    useEffect(() => {
        ...
        if (hasItems) {
            cartItems.forEach(item => {
                item && item.product && item.product.stock_status === 'OUT_OF_STOCK' && setHasOutOfStock(true);
                item &&
                    item.errors &&
                    item.errors.map(error => {
                        error.code === 'ITEM_QTY' && setHasOutOfStock(true);
                    });
            });
        }
    }, [hasItems, cartItems]);
 ...
 * @property {boolean} hasOutOfStock True if the cart has items that is out of stock. False otherwise.
 */
