...
import { REORDER_ITEMS } from './reOrderRow.gql';
...
export const useOrderRow = props => {
    ...
    const handleReorderItems = useCallback(async () => {
        try {
            ...
            if (
                ...
                data.reorderItems.cart.items &&
                data.reorderItems.cart.items.length > 0
            ) {
                addToast({
                ...
                addToast({
                    ...
                    message: formatMessage({
                        id: 'myAccount.reorderOutOfStock',
                        defaultMessage: 'Unsuccessfully action, some of the product is out-of-stock'
                    })
                });
            }
        } catch (e) {
    ...
    }, [addToast, formatMessage, orderNumber, reorderItems]);
