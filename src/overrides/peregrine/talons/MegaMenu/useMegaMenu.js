...
export const useMegaMenu = (props = {}) => {
    ...
    const megaMenuAllLinks = useMemo(() => {
        if (data && data.categoryList && data.categoryList[0]) {
            const navData = { ...data.categoryList[0] };
            navData.children = [...data.categoryList[0].children, ...otherHeaderLinks] || [];
            return processData(navData);
        }
        return {};
    }, [data, otherHeaderLinks, processData]);
    ...
    const handleOverflow = () => {
        const fullNav = props.overflowRef.current;
        const sibling = fullNav.nextElementSibling;

        const navX = isRtlDirection
            ? fullNav.getBoundingClientRect().x - PADDING
            : fullNav.getBoundingClientRect().x + fullNav.getBoundingClientRect().width;
        const siblingX = isRtlDirection
            ? sibling.getBoundingClientRect().x + sibling.getBoundingClientRect().width
            : sibling.getBoundingClientRect().x - PADDING;

        if (navX > 0 && siblingX > 0) {
            setIsOverflowing(isRtlDirection ? navX < siblingX : navX > siblingX);
        }
    };
    ...
    useEventListener(globalThis, 'resize', handleOverflow);
    ...
    useEffect(() => {
        const activeCategory = findActiveCategory(location.pathname, megaMenuAllLinks);
    ...
    }, [findActiveCategory, location.pathname, megaMenuAllLinks]);
     ...
     * @returns {void}
     */
    const { setShimmerType } = useInternalLink('category_page');
